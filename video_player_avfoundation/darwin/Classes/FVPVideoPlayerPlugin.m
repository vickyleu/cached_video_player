...
#import "SUPlayer.h"
#import "messages.g.h"
#import "Category/NSURL+SULoader.h"
...
- (SUPlayer *)playerWithPlayerItem:(AVPlayerItem *)playerItem { //AVPlayer
    return  [[SUPlayer alloc] initWithItem:playerItem];
...
#endif
    __strong typeof(self) sel = self;
    
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        AVPlayerItem *realItem = sel.player.realItem;
        
        //  _player.player.actionAtItemEnd = AVPlayerActionAtItemEndNone;
        ...
        if(sel){
            ...
            sel->_playerLayer = [AVPlayerLayer playerLayerWithPlayer:sel.player.player];
            [sel.flutterViewLayer addSublayer:sel->_playerLayer];
            [sel addObserversForItem:realItem player:sel.player.player];
            
        }
        
        
        [realItem.asset loadValuesAsynchronouslyForKeys:@[ @"tracks" ] completionHandler:assetCompletionHandler];
        NSLog(@"_player.player  %@",sel.player.player);
    });
...
- (void)observeValueForKeyPath:(NSString *)path
    ...
    NSLog(@"path::::%@ _eventSink:%@",path,_eventSink);
    if (context == timeRangeContext) {
        ...
        [self updatePlayingState];
        if ([[_player.player currentItem] isPlaybackLikelyToKeepUp]) {
            if (_eventSink != nil) {
        ...
        NSLog(@"rateContext called AVPlayerStatusReadyToPlay?=%@  isInitialized?=%@",player.status == AVPlayerStatusReadyToPlay?@"true":@"false",self.isInitialized?@"true":@"false");
        if(!self.isInitialized && player.status == AVPlayerStatusReadyToPlay){
            NSLog(@"rateContext called >???????");
            [player.currentItem addOutput:_videoOutput];
            [self setupEventSinkIfReadyToPlay];
            [self updatePlayingState];
        }
        if (_eventSink != nil) {
...
- (void)setupEventSinkIfReadyToPlay {
    if (_eventSink && !_isInitialized) {
        NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? ");
        AVPlayerItem *currentItem = self.player.player.currentItem;
        ...
        NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? _isInitialized_isInitialized ----111  tracks=%ld AVKeyValueStatusLoaded==%ld",(long)[asset statusOfValueForKey:@"tracks" error:nil] , AVKeyValueStatusLoaded);
        if ([asset statusOfValueForKey:@"tracks" error:nil] != AVKeyValueStatusLoaded) {
            void (^trackCompletionHandler)(void) = ^{
                
                NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? _isInitialized_isInitialized ----111222  tracks=%ld AVKeyValueStatusLoaded==%ld",(long)[asset statusOfValueForKey:@"tracks" error:nil] , AVKeyValueStatusLoaded);
        ...
        // a size.
        NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? _isInitialized_isInitialized 00");
        ...
        NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? _isInitialized_isInitialized 11");
        if (duration == 0) {
            return;
        }
        NSLog(@"setupEventSinkIfReadyToPlay   what the hell?? _isInitialized_isInitialized 22");
        _isInitialized = YES;
        _eventSink(@{
            @"event" : @"initialized",
            @"duration" : @(duration),
    ...
    NSLog(@"setupEventSinkIfReadyToPlay   _isInitialized:%@", _isInitialized?@"true":@"false");
}
...
- (int64_t)position {
    return FVPCMTimeToMillis([_player.player currentTime]);
}
...
- (int64_t)duration {
    ...
    return FVPCMTimeToMillis([[[_player.player currentItem] asset] duration]);
}

- (void)seekTo:(int64_t)location completionHandler:(void (^)(BOOL))completionHandler {
    CMTime previousCMTime = _player.player.currentTime;
    CMTime targetCMTime = CMTimeMake(location, 1000);
    CMTimeValue duration = _player.player.currentItem.asset.duration.value;
    ...
    [_player seekToTime:targetCMTime
      ...
      completionHandler:^(BOOL completed) {
        if (CMTimeCompare(self.player.player.currentTime, previousCMTime) != 0) {
...
- (void)setVolume:(double)volume {
    _player.player.volume = (float)((volume < 0.0) ? 0.0 : ((volume > 1.0) ? 1.0 : volume));
}

- (void)setPlaybackSpeed:(double)speed {
    // See https://developer.apple.com/library/archive/qa/qa1772/_index.html for an explanation of
    // these checks.
    if (speed > 2.0 && !_player.player.currentItem.canPlayFastForward) {
        if (_eventSink != nil) {
    ...
    if (speed < 1.0 && !_player.player.currentItem.canPlaySlowForward) {
        if (_eventSink != nil) {
            _eventSink([FlutterError errorWithCode:@"VideoError"
...
- (CVPixelBufferRef)copyPixelBuffer {
    ...
    if(CMTIME_IS_VALID(outputItemTime)){
...
- (void)removeKeyValueObservers {
    AVPlayerItem *currentItem = _player.player.currentItem;
    @try {
...
- (FVPTextureMessage *)onPlayerSetup:(FVPVideoPlayer *)player
    ...
    FlutterEventChannel *eventChannel = [FlutterEventChannel
                                         eventChannelWithName:[NSString stringWithFormat:@"flutter.io/videoPlayer/videoEvents%lld", textureId] binaryMessenger:_messenger];
    
    
    NSLog(@"StreamHandler player %lld",textureId);
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSLog(@"StreamHandler expectFrame %lld",textureId);
        [player expectFrame];
    });
...
- (FVPTextureMessage *)create:(FVPCreateMessage *)input error:(FlutterError **)error {
    ...
    if (input.asset) {
        ...
        NSURL* modifiedURL =[url specialSchemeURL];
        player = [[FVPVideoPlayer alloc] initWithURL:modifiedURL
                                        frameUpdater:frameUpdater
                                         displayLink:displayLink
...
- (void)seekTo:(FVPPositionMessage *)input
 ...
 completionHandler:^(BOOL finished) {
        dispatch_async(dispatch_get_main_queue(), ^{
            //          [player.frameUpdater displayLinkFired];
            completion(nil);
        });
    }];
}
